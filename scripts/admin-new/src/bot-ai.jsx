const BotAIPage = () => {
  ...
  const send = () => {
    ...
    const msg = testMsg;
    setConvo(c => [...c, { role: 'user', text: msg }]);
    setTestMsg('');
    setThinking(true);
    const delay = Math.floor(Math.random() * (maxDelay - minDelay)) + minDelay;
    setTimeout(() => {
      const responses = [
        'Vay, cok ilginc! Anlatsana biraz daha :)',
        'Yaa cidden mi? :D Ben de bayilirim oyle seylere.',
        'Hmm oyle mi dusunuyorsun? Ilk defa duydum boyle :)',
        'Cok tatlisan :) Yarin icin planin var mi?',
        'Aynen, ben de tam oyle hissediyorum.',
      ];
      setConvo(c => [...c, { role: 'bot', text: responses[Math.floor(Math.random() * responses.length)] }]);
      setThinking(false);
    }, delay);
  };
    ...
    <div className="page fade-in">
      ...
      <div className="card">
        <div className="card__head">
          <div><div className="card__title">Yanit Testi</div><div className="card__sub">Yapilandirmayi canli dene (lokal simulasyon)</div></div>
          <button className="btn btn--ghost btn--sm" onClick={() => setConvo([])}><Icon name="refresh" size={13} /> Sifirla</button>
        </div>
        <div className="card__body">
          <div style={{ background: 'var(--bg-2)', border: '1px solid var(--border)', borderRadius: 12, padding: 16, minHeight: 280, display: 'flex', flexDirection: 'column', gap: 8 }}>
            {convo.map((m, i) => (
            ...
            {convo.length === 0 && !thinking && <div className="empty">Test mesaji yazarak baslayabilirsin</div>}
          </div>
          <div className="row" style={{ marginTop: 12, gap: 8 }}>
            <input className="input" placeholder="Test mesaji yaz..." value={testMsg} onChange={e => setTestMsg(e.target.value)} onKeyDown={e => e.key === 'Enter' && send()} />
          ...
          <div style={{ marginTop: 10, fontSize: 11.5, color: 'var(--text-3)' }}>
            Test yaniti gercek API kullanmadan yerel olarak simule edilir &middot; Gecikme {minDelay}-{maxDelay}ms araliginda
          </div>
        </div>
      </div>
